...
export class Invoice {
  ...
  constructor(doc: jsPDF, config: VossConfig) {
    this.doc = doc;
    //TODO: change font?
    ...
    if(config.invoiceElements) {
      this.invoiceElements = config.invoiceElements;
    }

    this.maxWidth = (this.width - this.margin.left * 2);
    this.invoiceNr = config.invoiceNr;
  }

  set setSeller(seller: Person) {
    this.seller = seller;
  }
  ...
  set setPositions(positions: Position[]) {
    this.positions = positions;
  }

  get getPositions(): Position[] {
    return this.positions;
  }

  nextLine(yPosition?: number) {
  ...
  createHeader() {
    ...
    this.doc.setFontSize(10)
      ...
      .text(`${ this.buyer.address.street } ${ this.buyer.address.nr }`, this.margin.left, this.nextLine())
      .text('', this.margin.left, this.nextLine())
      .text(`${ this.buyer.address.code } ${ this.buyer.address.city }`, this.margin.left, this.nextLine())
  ...
  createBody() {
    ...
    this.lineCount++;
      ...
      let finalY = 0;
      let index = 0;
      let invoicePositions: Array<string[]> = this.positions.map( ( position: Position ) => {
        index++;
        //elements have to be strings for autoTable
        return [ 
          `${index}`,
          `${position.description}`,
          `${position.price.toFixed(2).toLocaleString().replace('.', ',')} €`,
          `${position.amount}`,
          `${(position.price * position.amount).toFixed(2).replace('.', ',')} €`, ];
      });
